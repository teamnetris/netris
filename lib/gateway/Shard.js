...
class Shard extends EventEmitter {
    ...
    constructor(id, client) {
        ...
        this.#onWSOpen = this.#onWSOpenUnbound.bind(this);
        this.#onWSMessage = this.#onWSMessageUnbound.bind(this);
        this.#onWSError = this.#onWSErrorUnbound.bind(this);
        this.#onWSClose = this.#onWSCloseUnbound.bind(this);
    ...
    * @arg {String} [activities[].state] The state of the activity - if using a custom status, this is the text to be shown to the users
    */
    ...
    wsEvent(packet) {
        switch(packet.t) { /* eslint-disable no-redeclare */ // (╯°□°）╯︵ ┻━┻
            case "AUTO_MODERATION_ACTION_EXECUTION": {
                ...
                * @prop {Object} action The execution action
                */
            ...
            case "AUTO_MODERATION_RULE_CREATE": {
                ...
                */
                this.emit("autoModerationRuleCreate", guild, new AutoModerationRule(packet.d, this.client));
                break;
            }
            case "AUTO_MODERATION_RULE_DELETE": {
                ...
                */
                this.emit("autoModerationRuleDelete", guild, new AutoModerationRule(packet.d, this.client));
                break;
            }
            case "AUTO_MODERATION_RULE_UPDATE": {
                ...
                * @prop {Guild} guild The guild associated with the rule
                * @prop {AutoModerationRule?} rule The old rule, null if uncached
                * @prop {AutoModerationRule} newRule The new rule
                */
                this.emit("autoModerationRuleUpdate", guild, null, new AutoModerationRule(packet.d, this.client));
                break;
            }
            case "PRESENCE_UPDATE": {
                if(packet.d.user.username !== undefined) {
                    ...
                    let oldUser = null;
                    if(user && (user.username !== packet.d.user.username || user.discriminator !== packet.d.user.discriminator || user.avatar !== packet.d.user.avatar || user.globalName !== packet.d.user.global_name)) {
                        oldUser = {
                    ...
                    if(!user || oldUser) {
                        ...
                        * @prop {String?} oldUser.globalName The globally visible display name of the user
                        */
            ...
            case "MESSAGE_UPDATE": {
                ...
                * @prop {Array<Object>} oldMessage.components An array of message components
            ...
            case "MESSAGE_REACTION_ADD": {
                ...
                if(message) {
                    ...
                    if(packet.d.message_author_id) {
                        message.author = this.client.users.get(packet.d.message_author_id) ?? {id: packet.d.message_author_id};
                    }
                ...
                * @prop {Message | Object} message The message object. If the message is not cached, this will be an object with `id`, `channel`, `author` (if present in the payload), and if inside a guild, `guildID` keys. If the channel is not cached, channel key will be an object with only an id. If the author is not cached, the author key will be an object with only an ID. `guildID` will be present if the message was sent in a guild channel. No other property is guaranteed
            ...
            case "GUILD_UPDATE": {
                ...
                * @prop {Number?} oldGuild.maxStageVideoChannelUsers The max number of users allowed in a stage video channel
            ...
            case "GUILD_BAN_ADD": {
                ...
                * @prop {Guild} guild The guild - if uncached, this will be an object with the guild's ID. No other properties are guaranteed
            ...
            case "GUILD_ROLE_UPDATE": {
                ...
                * @prop {Number} oldRole.flags Role flags. See [Discord's documentation](https://discord.com/developers/docs/topics/permissions#role-object-role-flags) for a list of them
            ...
            case "CHANNEL_UPDATE": {
                ...
                if(oldType === packet.d.type) {
                    channel.update(packet.d, this.client);
                } else {
                ...
                * @prop {Array<Object>} oldChannel.availableTags The available tags of the channel (forum channels only)
                * @prop {Number} oldChannel.bitrate The bitrate of the channel (voice channels only)
                * @prop {Number} oldChannel.defaultAutoArchiveDuration The default duration of newly created threads in minutes to automatically archive the thread after inactivity (60, 1440, 4320, 10080)
                * @prop {Object} oldChannel.defaultReactionEmoji The emoji to show as the reaction button
                * @prop {Number} oldChannel.defaultSortOrder The default thread sorting order
                * @prop {Number} oldChannel.defaultThreadRateLimitPerUser The initial ratelimit of the channel to use on newly created threads, in seconds. 0 means no ratelimit is enabled
            ...
            case "INTERACTION_CREATE": {
                ...
                * @prop {CommandInteraction | ComponentInteraction | AutocompleteInteraction | ModalSubmitInteraction} interaction The Interaction that was created
                */
            ...
            case "GUILD_AUDIT_LOG_ENTRY_CREATE": {
                ...
                /**
                 * Fired when an audit log entry is created
                 * @event Client#guildAuditLogEntryCreate
                 * @prop {GuildAuditLogEntry} entry The audit log entry
                 */
                this.emit("guildAuditLogEntryCreate", new GuildAuditLogEntry(packet.d, guild));
                break;
            }
            case "INTEGRATION_CREATE": {
                 ...
                 * @event Client#integrationCreate
                 * @prop {Guild} guild The guild this integration belongs to
                 * @prop {GuildIntegration} integration The integration
                 */
                this.emit("integrationCreate", guild, new GuildIntegration(packet.d, guild));
                break;
            }
            case "INTEGRATION_UPDATE": {
                ...
                /**
                 * Fired when a guild integration is updated
                 * @event Client#integrationUpdate
                 * @prop {Guild} guild The guild this integration belongs to
                 * @prop {GuildIntegration} integration The integration
                 */
                this.emit("integrationUpdate", guild, new GuildIntegration(packet.d, guild));
                break;
            }
            case "INTEGRATION_DELETE": {
                ...
                /**
                 * Fired when a guild integration is removed
                 * @event Client#integrationDelete
                 * @prop {Guild} guild The guild this integration belongs to. If uncached, this will be an object with an ID key. No other property is guaranteed
                 * @prop {Object} integration The data about the deleted integration
                 * @prop {String?} integration.applicationID The OAuth application ID associated with this integration
